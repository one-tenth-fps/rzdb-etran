    ...
    try:
        while True:
            try:
                ...
                for row in rows:
                    request_body, request_type, request_id, request_priority = None, row.TypeID, row.ID, row.Priority
                    logging.info(
                    ...
                    try:
                        ...
                        logging.warning(
                            f'{task_name} id={request_id} {repr(e)}')
                        await queue_out.put(ResponsePacket(
                            request_id, is_error=True, body=repr(e).encode(), request_packet=None))

                    # отправляем в очередь обработки запросов
                    if request_body is not None:
                        request_packet = RequestPacket(
                            request_priority, request_id, request_body, 0)
...
async def main():
    ...
    producers = [asyncio.create_task(utils.rerun_on_exception(
        producer_db, queue_in, queue_out, sleep_for=config.SLEEP_ON_DISCONNECT))]
    workers = [asyncio.create_task(worker(queue_in, queue_out), name=f'worker-{i}')
               for i in range(config.WORKERS_COUNT)]
    consumers = [asyncio.create_task(utils.rerun_on_exception(
